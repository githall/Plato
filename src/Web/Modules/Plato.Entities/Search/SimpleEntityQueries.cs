...
namespace Plato.Entities.Search
{
    ...
    public class SimpleEntityQueries<TModel> : IFederatedQueryProvider<TModel> where TModel : class
    {

        protected readonly IFullTextQueryParser _fullTextQueryParser;

        public SimpleEntityQueries(IFullTextQueryParser fullTextQueryParser)
        {
        ...
        public IEnumerable<string> Build(IQuery<TModel> query)
        {
            ...
            if (query.GetType() != typeof(SimpleEntityQuery<TModel>))
            {
                return null;
            }
            
            // Convert to correct query type
            var typedQuery = (SimpleEntityQuery<TModel>)Convert.ChangeType(query, typeof(SimpleEntityQuery<TModel>));
        ...
        IList<string> BuildSqlQueries(SimpleEntityQuery<TModel> query)
        {
        ...
        IList<string> BuildFullTextQueries(SimpleEntityQuery<TModel> query)
        {
