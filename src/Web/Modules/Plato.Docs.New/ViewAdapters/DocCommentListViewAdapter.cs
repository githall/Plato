...
namespace Plato.Docs.New.ViewAdapters
{
    ...
    public class DocCommentListViewAdapter : ViewAdapterProviderBase
    {   

        public IHtmlLocalizer T { get; }

        public DocCommentListViewAdapter(IHtmlLocalizer<DocListItemViewAdapter> localizer)
        {

            T = localizer;
            ViewName = "DocCommentList";
        ...
        public override async Task<IViewAdapterResult> ConfigureAsync(string viewName)
        {
            ...
            {
                v.AdaptModel<EntityViewModel<Doc, DocComment>>(model =>
                {
                    ...
                    var adapters = new[]
                    {
                        new TagHelperAdapter(ViewName, "title", (context, output) =>
                        {
                            ...
                            output.PostElement.SetHtmlContent(
                                        $"<span class=\"badge badge-primary ml-2\">{T["Doc Comment List View Adapter"].Value}</span>");
                        })
                    };
