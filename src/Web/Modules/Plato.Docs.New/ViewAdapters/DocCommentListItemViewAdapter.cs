...
namespace Plato.Docs.New.ViewAdapters
{
    ...
    public class DocCommentListItemViewAdapter : ViewAdapterProviderBase
    {   
        ...
        public override async Task<IViewAdapterResult> ConfigureAsync(string viewName)
        {
            ...
            {
                v.AdaptModel<EntityReplyListItemViewModel<Doc, DocComment>>(model =>
                {
                    ...
                    var adapters = new[]
                    {
                        new TagHelperAdapter("title", (context, output) =>
                        {
                            output.PostElement.SetHtmlContent(
                                        $"<span class=\"badge badge-primary ml-2\">{T["Doc Comment List Item View Adapter"].Value}</span>");
                        })
                    };
