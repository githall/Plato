                           ...
                           if (lastVisit != null)
                            {
                                ...
                                if (model.Entity.LastReplyAfter(lastVisit))
                                {
                                    output.PreElement.SetHtmlContent(
                                        $"<span class=\"text-primary mr-2 smaller\" data-toggle=\"tooltip\" title=\"{T["This doc has new replies"].Value}\"><i class=\"fa fa-circle\"></i></span>");
                                }
                                else
                                {
                                    // Modified
                                    ...
                                    {
                                        output.PreElement.SetHtmlContent(
                                            $"<span class=\"text-primary mr-2 smaller\" data-toggle=\"tooltip\" title=\"{T["This doc has been updated since it was last read"].Value}\"><i class=\"fa fa-circle\"></i></span>");
                                    }
                                }
                            }
                            else
                            {
                                // Unread
                                output.PreElement.SetHtmlContent(
                                    $"<span data-toggle=\"tooltip\" title=\"{T["You've not read this doc"].Value}\" class=\"text-primary mr-2 smaller\"><i class=\"fa fa-circle\"></i></span>");
                            }
